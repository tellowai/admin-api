...
exports.validateTemplateTopByGenerationQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.templateTopByGenerationSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateTemplatePerformanceQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.templatePerformanceAnalyticsSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateCreditsAnalyticsQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.creditsAnalyticsSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateTechHealthAnalyticsQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.techHealthAnalyticsSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateOrdersFunnelClickhouseQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.ordersFunnelClickhouseSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateGrowthMetricsOverviewQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.growthMetricsOverviewSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
...
exports.validateSearchAnalyticsQuery = function(req, res, next) {
  const payload = req.query;
  const payloadValidation = validationCtrl.validate(analyticsSchema.searchAnalyticsSchema, payload);
  if (payloadValidation.error && payloadValidation.error.length) {
    return res.status(HTTP_CODES.BAD_REQUEST).json({
      message: req.t('validation:VALIDATION_FAILED'),
      data: payloadValidation.error
